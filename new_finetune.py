...
from data_utils.score import SegmentationMetric
...
def main(train_data_path, train_label_path, nb_epoch, save_path, start_path=None, batch_size=4, lr=0.001, depth_est = True, color_seg = False,
    ...
    if model_type == 'hourglass':
        ...
        if depth_est and not color_seg:
            ...
            model = DPTSegmentationModel(
                256,
                path='/home/soumyajit/DPT_inference/weights/07M-27D_02h-46m-30s_best.pth',
                backbone="vitb_rn50_384",
            )
    ...
    history = fit(model = model, train = dataset_train, criterion= criterion, optimizer = optimizer, save_path = save_path, batch_size = batch_size,
                  nb_epoch = nb_epoch, depth_est = depth_est, color_seg = color_seg, validation_data = dataset_valid, ignore_index = ignore_index, loss_log_dir = loss_log_dir, seg_metric=metric)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data_path', default='/home/soumyajit/Dataset/images/finetune')
    parser.add_argument('--train_label_path', default='/home/soumyajit/Dataset/annotations/finetune')
